...
| [ ] | Nume anterior / schimbat si nume actual | de revizuit; partial; implementat | Formula mentionata in `docs/formule-calcul.md`; implementata comparatia in script; nu are capitol dedicat complet. | Crea/valida capitol dedicat cu metoda, exemple si limite. |
...
| [ ] | Calea destinului | de revizuit; documentat extins; neimplementat separat | Metoda si exemple extinse: suma cifrelor datei, neredusa complet; echivalenta cu `vibratia caii destinului`; interpretare `ce/cum`; categorii 4-9, 10-19, 20-29, 30-39, 40-48 integrate din MDI. | Implementa separat in script regula `ce/cum`, categoria mare si legatura cu vibratia destinului. |
...
| [x] | Soarta si destin / tema vietii | de revizuit; documentat; implementat | Metoda, semnificatii si exemple comasate direct in `knowledge_base/calcule/soarta-si-destin/`; `tema vietii` ramane denumire alternativa. | Revizuire editoriala si validare ca sinteza intre soarta si destin. |
...
| [x] | Karma zilei de nastere | documentat; implementat | Metoda si exemple preluate din bibliografia 05; implementat in script. | Completa interpretari extinse pe fiecare zi daca intra in v1. |
| [x] | Karma lunii de nastere | documentat; implementat | Metoda si exemple preluate din bibliografia 06; implementat in script. | Completa interpretari extinse pe fiecare luna daca intra in v1. |
| [x] | Karma din calea destinului | documentat; implementat | Metoda si exemple preluate din bibliografia 07; implementat in script. | Completa interpretari extinse si avertismente pe fiecare numar daca intra in v1. |
| [x] | Karma neamului | alias, nu calcul separat | Eliminat ca director/calcul separat; termenul este documentat la numarul ereditar karmic / numarul neamului. | Fara task separat. |
